...
// import {APIUser, serverPath} from "../rest";
...
export class UserProfileElement extends App.View {
    ...
    render() {
        ...
        const renderList = (s: string) => html`<dd>${s}</dd>`;


        // ${this._renderAvatar()}
        // this goes right below <section>
        ...
        <dl>
          ...
          <dt>Home City</dt>
            <dd>${city}</dd>
          <dt>Genres</dt>
            ${genres.map(renderList)}
          <dt>Mediums</dt>
            ${mediums.map(renderList)}
          <dt>Artists</dt>
            ${artists.map(renderList)}
        </dl>
      </section>
    `;
    }
